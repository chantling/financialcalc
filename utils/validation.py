...
def validate_growth_rates(
    ...
    if company_revenue and company_revenue > 100_000_000_000:  # $100B+
        max_growth = max(growth_rates[:5]) if len(growth_rates) >= 5 else max(growth_rates)
        if max_growth > 25:
            warnings.append(
...
def validate_assumptions(
    ...
    warnings.extend(validate_growth_rates(
        growth_rates, historical_cagr, company_revenue
    ))
    
    warnings.extend(validate_terminal_multiple(
...
def calculate_confidence_score(
    ...
    if terminal_multiple > 25:
        score -= 15
        risk_factors.append(f"Terminal multiple ({terminal_multiple}x) is above typical range (8-25x)")
    elif terminal_multiple < 8:
        score -= 10
        risk_factors.append(f"Terminal multiple ({terminal_multiple}x) is below typical range (8-25x)")
    
    # Discount rate reasonableness
    if discount_rate < 6 or discount_rate > 15:
        score -= 10
        risk_factors.append(f"Discount rate ({discount_rate}%) outside standard range (6-15%)")
